...
import tmit.bme.telkicar.logic.LogicHelper
...
class IntegerProgrammingMatcher @Autowired constructor(
    private val spatialRepository: SpatialRepository,
    private val ipSolver: IpSolver<String>,
) : ParticipantMatcher {
    ...
    private fun assembleResult(): Map<Fuvar, List<Igeny>> =
        pairings.mapValues { (driver, driversPassengers) ->
            driversPassengers.keys.filter {
                ipSolver.getResult(pairings[driver]!![it]!!) == 1
            }
        }
    ...
    private fun solve() {
        logger.info("variables count: ${ipSolver.variablesCount}")
        logger.info("constraints count: ${ipSolver.constraintsCount}")
        val result = ipSolver.solve()
        saveRoutes()
    ...
    private fun saveRoutes() {
        if (appContextHelper.isMatchingToBeSavedToDb)
            assembleUsersRoutes().forEach { (user, routeAndActionPoints: Pair<Route, MutableList<ActionPoint>>) ->
    ...
    private fun assembleUsersRoutes() = usersPaths.mapValues { (user, pathsForUser) ->
        ...
        while (currentPoint != destinationPoint) {
            val actionPair = pathsForUser.filter { (actionPointPair, variable) ->
                actionPointPair.first == currentPoint && ipSolver.getResult(variable) == 1
    ...
    private fun pathValidityForPassenger(
        ...
        ipSolver.startConstraint(ConstraintType.Eq)
        paths.filter { it.key.first == actionPoint }.values.forEach { // outgoing edges
            ipSolver.addConstraintVariable(it, 1)
        }
        paths.filter { it.key.second == actionPoint }.values.forEach { // incoming edges
            ipSolver.addConstraintVariable(it, -1)
        }
        if (actionPoint == startPoint)
            tieWithPairingVariables(passenger, -1)
        if (actionPoint == destinationPoint)
            tieWithPairingVariables(passenger, 1)
        ipSolver.registerConstraint(rhs = 0)
    }

    private fun tieWithPairingVariables(passenger: ActionPoint, coefficient: Long) {
        pairings.forEach { (_, passengers) ->
            passengers[passenger]?.let {
                ipSolver.addConstraintVariable(it, coefficient) }
        }
    }
    ...
    private fun pathValidityForDriver(
        ...
        val outgoingEdges = paths.filter { it.key.first == actionPoint }.values
        outDegreeMax1Constraint(outgoingEdges)
        locationDegreeConstraint(actionPoint, startPoint, destinationPoint, outgoingEdges, paths)
    }

    private fun locationDegreeConstraint(
        actionPoint: ActionPoint,
        startPoint: ActionPoint,
        destinationPoint: ActionPoint,
        outgoingEdges: Collection<String>,
        paths: Map<Pair<ActionPoint, ActionPoint>, String>
    ) {
        ...
        ipSolver.registerConstraint(rhs = degree)
    }

    private fun outDegreeMax1Constraint(outgoingEdges: Collection<String>) {
        ipSolver.startConstraint(ConstraintType.Lt)
        outgoingEdges.forEach {
            ipSolver.addConstraintVariable(it, 1)
        }
        ipSolver.registerConstraint(rhs = 1)
    }
    ...
    private fun addVehicleCapacitiesConstraint() {
        pairings.entries.forEachIndexed { i, driversPairings ->
            ...
            driversPairings.value.entries.forEach { (igeny, variable) ->
                ipSolver.addConstraintVariable(variable, igeny.utasokSzama)
            }
            ipSolver.registerConstraint(rhs = drivers[i].szabadHely)
        }
    }
        ...
        passengerPlans.forEach { passenger ->
            ...
            pairings.values.forEach { driversPairings ->
                if (driversPairings.containsKey(passenger))
                    ipSolver.addConstraintVariable(driversPairings[passenger]!!, 1)
            }
            ipSolver.registerConstraint(rhs = 1)
        }
    }
        ...
        pairings.forEach { (driver, driversPairings) ->
            driversPairings.forEach { (passenger, areTheyPaired) ->
                ...
                passengersPaths.keys.forEach { passengerActionPoints ->
                    ...
                    ipSolver.apply {
                        startConstraint(ConstraintType.Lt)
                        addConstraintVariable(areTheyPaired, 1)
                        addConstraintVariable(passengersPaths[passengerActionPoints]!!, 1)
                        if (driversPaths.containsKey(passengerActionPoints))
                            addConstraintVariable(driversPaths[passengerActionPoints]!!, -1)
        ...
        usersPaths = userActionPoints.associateWith { user ->
            ...
            paths
                ...
                .mapValues {
                    ...
                    ipSolver.addCostBoolVar(pathVar, pathWeight * (paths[it.key]?.lengthInMeters ?: 0))
                    pathVar
                }
        }
    }
            ...
            possiblePassengersForDrivers.entries.associate { passengersForDriver ->
                passengersForDriver.key to
                    passengersForDriver.value.second.associateWith {
                        ...
                        val pairingVarCoeff = pairingWeight + it.utasokSzama * numberOfPeoplePerPassengerRequestWeight
                        ipSolver.addCostBoolVar(pairingVar, pairingVarCoeff)
                        pairingVar
                    }
            }
